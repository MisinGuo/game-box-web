...
erDiagram
    %% 网站核心
    gb_sites ||--o{ gb_game_boxes : "拥有"
    gb_sites ||--o{ gb_games : "拥有"
    gb_sites ||--o{ gb_articles : "拥有"
    gb_sites ||--o{ gb_drama_vendors : "拥有"
    gb_sites ||--o{ gb_dramas : "拥有"
    gb_sites ||--o{ gb_prompt_templates : "拥有"
    
    %% 游戏盒子-游戏多对多关系
    gb_game_boxes ||--o{ gb_box_game_relations : "包含游戏"
    gb_games ||--o{ gb_box_game_relations : "被盒子收录"
    
    %% 游戏相关扩展
    gb_games ||--o{ gb_game_activities : "活动"
    gb_games ||--o{ gb_game_servers : "开服"
    gb_games ||--o{ gb_game_giftcodes : "礼包"
    gb_games ||--o{ gb_game_materials : "素材"
    
    %% 文档相关
    gb_documents ||--o{ gb_document_game_relations : "关联游戏"
    gb_documents ||--o{ gb_document_tags : "标签"
    gb_documents }|--|| gb_categories : "所属分类"
    gb_documents }|--o| gb_storage_configs : "存储配置"
    
    %% 文章相关（AI生成）
    gb_articles ||--o{ gb_article_generations : "生成记录"
    gb_articles ||--o{ gb_article_product_relations : "关联产品"
    gb_articles ||--o{ gb_article_resources : "资源附件"
    gb_articles }|--o| gb_prompt_templates : "使用模板"
    gb_articles }|--o| gb_storage_configs : "资源存储配置"
    gb_article_generations }|--|| gb_ai_platforms : "使用平台"
    gb_article_generations }|--o| gb_prompt_templates : "使用模板"
    gb_article_resources }|--o| gb_storage_configs : "存储位置"
    
    %% 提示词模板
    gb_prompt_templates ||--o{ gb_articles : "生成文章"
    gb_prompt_templates ||--o{ gb_article_generations : "生成记录"
    
    %% 存储迁移相关
    gb_storage_migration_rules ||--o{ gb_storage_migration_logs : "执行日志"
    gb_article_resources ||--o{ gb_storage_migration_logs : "迁移记录"
    
    %% 短剧相关
    gb_drama_vendors ||--o{ gb_dramas : "拥有"
    gb_dramas ||--o{ gb_article_product_relations : "被文章关联"
    
    %% 游戏与文章关联
    gb_games ||--o{ gb_document_game_relations : "被文档关联"
    gb_games ||--o{ gb_article_product_relations : "被文章关联"
    gb_game_boxes ||--o{ gb_article_product_relations : "被文章关联"
    
    %% 统计相关
    gb_documents ||--o{ gb_page_views : "浏览记录"
    gb_articles ||--o{ gb_page_views : "浏览记录"
    
    %% 搜索相关
    gb_search_logs ||--o{ gb_search_clicks : "点击记录"
```
...
flowchart TB
    ...
    subgraph 游戏层
        GAME[游戏<br/>gb_games]
        GAME --> |折扣信息| DISCOUNT[首充/续充折扣<br/>国内/海外]
        GAME --> |设备支持| DEVICE[安卓/iOS/双端]
    end
    
    subgraph 游戏扩展数据
        GAME --> ACTIVITY[活动<br/>gb_game_activities]
        GAME --> SERVER[开服<br/>gb_game_servers]
        GAME --> GIFTCODE[礼包<br/>gb_game_giftcodes]
        GAME --> MATERIAL[素材<br/>gb_game_materials]
    end
    
    subgraph 关联层
        BOX <-->|gb_box_game_relations| GAME
        REL[盒子-游戏关联]
        REL --> |特定折扣| SPEC_DISCOUNT[盒子内游戏折扣]
        REL --> |推广链接| PROMO[下载链接/推广网站]
        REL --> |素材| POSTER[二维码/海报/推广语]
    end
...
### 2.2 对象存储配置表 (gb_storage_configs)

支持多种对象存储方式：GitHub仓库、MinIO、Cloudflare R2等。

```sql
CREATE TABLE `gb_storage_configs` (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT '主键ID',
  `name` varchar(100) NOT NULL COMMENT '配置名称',
  `storage_type` varchar(20) NOT NULL COMMENT '存储类型：github/minio/r2/oss/cos',
  ...
  `allowed_extensions` varchar(500) DEFAULT 'md,json,txt,jpg,png,gif,webp' COMMENT '允许的文件扩展名',
...
| R2 | `https://{account_id}.r2.cloudflarestorage.com/{bucket}/{path}` | Cloudflare边缘存储 |
...
CREATE TABLE `gb_github_repos` (
  ...
  `updated_at` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_user_repo` (`user_id`, `repo_type`),
...
### 4.5 文章资源表 (gb_article_resources)
...
CREATE TABLE `gb_article_resources` (
  ...
  `markdown_ref` varchar(500) DEFAULT NULL COMMENT 'Markdown引用格式（如 ![alt](url)）',
...
### 4.6 存储迁移规则表 (gb_storage_migration_rules)
...
### 4.7 存储迁移日志表 (gb_storage_migration_logs)
...
### 4.8 文章-产品关联表 (gb_article_product_relations)
...
| 25 | gb_search_logs | 搜索日志表 | product_analytics |
| 26 | gb_page_views | 页面浏览表 | product_analytics |
| 27 | gb_daily_stats | 每日统计汇总表 | product_analytics |
| 28 | gb_hot_keywords | 热门搜索词表 | product_analytics |
| 29 | gb_ai_generation_stats | AI生成统计表 | product_analytics |
...
graph TB
    ...
    subgraph 产品层
        GAMEBOX[gb_game_boxes<br/>游戏盒子]
        GAME[gb_games<br/>游戏]
        VENDOR[gb_drama_vendors<br/>短剧厂商]
    ...
    subgraph 内容层
        ...
        ART_RES[gb_article_resources<br/>文章资源]
    end
    
    subgraph 配置层
        STORAGE[gb_storage_configs<br/>存储配置]
        AI[gb_ai_platforms<br/>AI平台]
        PROMPT[gb_prompt_templates<br/>提示词模板]
        MIGRATE[gb_storage_migration_rules<br/>迁移规则]
    end
    ...
    ARTICLE -->|使用| PROMPT
    GAME -.->|被关联| ART_PROD
    GAMEBOX -.->|被关联| ART_PROD
    DRAMA -.->|被关联| ART_PROD
    
    ARTICLE -->|生成记录| ART_GEN
    ART_GEN -->|使用| AI
    ART_GEN -->|使用| PROMPT
