...
graph TB
    subgraph "边缘网络 - 多推广站点"
        W1[游戏推广站 A<br/>Next.js + Cloudflare Workers]
        W2[游戏推广站 B<br/>Next.js + Cloudflare Workers]
        W3[短剧推广站<br/>Next.js + Cloudflare Workers]
    end
    
    subgraph "统一管理平台"
        Admin[管理后台<br/>Vue3 + Element Plus]
        Backend[若依 Boot 后端<br/>ruoyi-admin]
    end
...
graph TB
    ...
    subgraph "数据层"
        ...
        N[(Redis)]
        O[(ElasticSearch)]
        P[(对象存储<br/>R2/OSS/COS)]
    end
    
    subgraph "外部服务"
        Q[GitHub Actions]
        R[AI 平台<br/>OpenAI/Claude/DeepSeek]
        S[图片搜索<br/>Unsplash/Pixabay]
    end
...
sequenceDiagram
    ...
    participant Site as 推广站点<br/>(边缘)
...
graph TB
    ...
    subgraph "外部服务"
        N[AI 平台<br/>OpenAI/Claude/DeepSeek]
        O[图片搜索<br/>Bing/Unsplash/Pixabay]
        P[对象存储<br/>R2/OSS/COS]
    end
...
sequenceDiagram
    ...
    participant ImageAPI as 图片搜索 API
    participant Storage as 对象存储

    Note over User,Storage: 一次性配置阶段
    User->>Admin: 1. 登录并授权 GitHub
    Admin->>GitHub: OAuth 认证
    GitHub-->>Admin: 返回 Access Token
    Admin->>Backend: 保存 Token
    ...
    User->>Admin: 3. 配置 AI API Key
    Admin->>Backend: 加密保存
    Backend->>GitHub: PUT /repos/:repo/actions/secrets
    
    User->>Admin: 4. 配置图片服务
    Admin->>Backend: 保存图片搜索 API Key
    Backend->>GitHub: 设置 IMAGE_SEARCH_API_KEY Secret
    
    User->>Admin: 5. 上传图片池
    Admin->>Backend: 上传图片/配置
    Backend->>Storage: 保存图片池 JSON
    
    User->>Admin: 6. 配置定时任务
    Admin->>Backend: 保存 Cron 表达式
    
    Note over User,Storage: 每日自动执行阶段
    Backend->>Backend: 定时任务触发 (如每天凌晨2点)
    Backend->>GitHub: POST /repos/:repo/actions/workflows/:id/dispatches
    
    GitHub->>Action: 启动 Workflow
    Action->>Action: 读取模板和配置
    Action->>AI: 调用 AI 生成文章
    AI-->>Action: 返回文章内容
    
    loop 处理图片占位符
        Action->>Action: 解析 IMAGE_SEARCH 占位符
        Action->>ImageAPI: 搜索图片
        ImageAPI-->>Action: 返回图片列表
        Action->>Storage: 下载并上传图片
        Storage-->>Action: 返回 CDN URL
        Action->>Action: 替换占位符
    end
    
    Action->>GitHub: git commit & push
    Action->>Backend: POST /webhook (完成回调)
    Backend->>Backend: 更新生成记录
    Backend->>Admin: 推送通知
    Admin->>User: 显示生成结果
```
